...
const GAME_ID = "cube-smash";
let PLAYER_NAME = "";
let db = null;

// simple write throttle so you don't spam firestore too fast
let _pendingPoints = 0;
...
async function _flushPointsNow(pointsToAdd) {
  if (!db) return;
  if (!PLAYER_NAME) return;

  const now = Date.now();

  // ✅ your aggregate leaderboard collection (matches screenshot)
  const aggRef = db.collection("scores_aggregate").doc(PLAYER_NAME);
  await aggRef.set(
    {
      playerName: PLAYER_NAME,
      totalScore: firebase.firestore.FieldValue.increment(pointsToAdd),
      updatedAt: now,
    },
    { merge: true }
  );
  ...
  await gameRef.set(
    {
      ...
      updatedAt: now,
      games: {
        [GAME_ID]: firebase.firestore.FieldValue.increment(pointsToAdd),
      },
    },
    { merge: true }
  );
...
// Add points, but batch them to reduce writes
function addPointsToFirestore(points) {
  try {
    if (!db) return;
    if (!PLAYER_NAME) return;
    ...
    // flush every ~700ms or when big
    if (_pendingPoints >= 20) {
      const pts = _pendingPoints;
      _pendingPoints = 0;
      _flushPointsNow(pts).then(() => {
        console.log(`✅ +${pts} points saved (batched) for ${PLAYER_NAME}`);
    ...
    console.error("❌ addPointsToFirestore error:", e);
  }
}
...
function showMenu(node) { node.classList.add('active'); }
function hideMenu(node) { node.classList.remove('active'); }
        ...
        if (!target.hit) {
          ...
          if (pointerSpeedScaled > minPointerSpeed) {
            ...
            if (target.health <= 0) {
              ...
              // ✅ ✅ ✅ Every cube smashed = +2 points to Firestore
              addPointsToFirestore(2);
